...
#    img_vec={k.split('/')[-2]:v for k,v in img_vec.items()}
    img_vec = {}
    for k, v in original_img_vec.items():
        parts = k.split('/')
        if len(parts) > 2:
            key = parts[-2]  # 获取倒数第二部分
        else:
            key = k  # 或者设定一个默认值，或者跳过这个键值对
        img_vec[key] = v
...
if __name__ == '__main__':
    get_img_vec_array(0, img_vec_path='data/analogy/analogy_best_img.pickle', eutput_file='data/analogy/img_vec_id_analogy_vit.pickle')
